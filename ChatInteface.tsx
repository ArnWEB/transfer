...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ user, token, onLogout }) => {
  ...
  const [currentDialogConfig,setCurrentDialogConfig] =useState<DialogConfig | null>(null);
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex overflow-hidden relative">
      ...
      {/* Main Chat Area */}
      <div className={`${isResearchMode ? 'flex-1' : 'flex-1'} flex flex-col transition-all duration-300 ${showDialog ? 'mr-96' : ''}`}>
        {/* Header */}
        ...
        <div className={`flex-1 overflow-y-auto p-4 lg:p-6 space-y-6 bg-gradient-to-b from-gray-900/50 to-gray-800/50 pb-24 ${isResearchMode ? 'lg:border-r lg:border-r-cyan-500/10' : ''}`}>
          {messages.map((message, index) => (
            ...
            >
              ...
              {/* Dialog Trigger Button - Now inside message */}
              {message.role === 'assistant' && message.dialogConfig && (
                <div className="mt-3 pt-3 border-t border-cyan-500/20">
                  ...
                  >
                    ...
                    <span className="text-sm font-medium">Open Interactive Form</span>
                  </button>
                  <p className="text-xs text-gray-400 mt-2">
                    Click to open the form, or continue chatting normally
                  </p>
                </div>
              )}
            </div>
        ...
        {/* Input Area */}
        <div className={`fixed bottom-0 ${isResearchMode ? 'left-0 right-1/2 lg:left-48' : `left-0 lg:left-64 ${showDialog ? 'right-96' : 'right-0'}`} bg-gray-900/95 backdrop-blur-md border-t border-cyan-500/20 p-4 lg:p-6 transition-all duration-300`}>
      ...
      {isResearchMode && (
        <div className="w-1/2 flex flex-col bg-gradient-to-br from-purple-900/20 via-gray-800 to-indigo-900/20 border-l border-purple-500/20">
          ...
          {/* Research Input Area */}
          <div className={`fixed bottom-0 left-1/2 ${showDialog ? 'right-96' : 'right-0'} bg-gradient-to-r from-purple-900/95 to-indigo-900/95 backdrop-blur-md border-t border-purple-500/20 p-4 lg:p-6 transition-all duration-300`}>
            <div className="max-w-4xl mx-auto">
              <div className="flex items-end space-x-4">
                <div className="flex-1 relative">
                  <input
      ...
      {/* Side Drawer Dialog */}
      {/* Non-overlay Side Drawer */}
      <div className={`fixed top-0 right-0 h-full bg-gradient-to-br from-gray-900 via-gray-800 to-black border-l border-cyan-500/30 shadow-2xl shadow-cyan-500/10 transform transition-transform duration-300 ease-in-out z-40 ${
        showDialog ? 'translate-x-0' : 'translate-x-full'
      } w-96 overflow-y-auto`}>
        {currentDialogConfig && (
          <>
            {/* Header */}
            <div className="sticky top-0 bg-gradient-to-r from-gray-900 to-gray-800 border-b border-cyan-500/20 p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-cyan-500/25">
                    <Shield className="w-5 h-5 text-white" />
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-white tracking-tight">{currentDialogConfig.title}</h2>
                    {currentDialogConfig.description && (
                      <p className="text-sm text-gray-400 mt-1">{currentDialogConfig.description}</p>
                    )}
                  </div>
                </div>
                <button
            ...
            <form onSubmit={(e) => {
              ...
              currentDialogConfig.fields.forEach(field => {
              ...
              {currentDialogConfig.fields.map((field) => (
                <div key={field.id} className="space-y-2">
                  ...
                  {field.type === 'text' || field.type === 'email' || field.type === 'number' ? (
                    <input
                      ...
                      className="w-full px-4 py-3 bg-gray-800/70 border border-cyan-500/30 text-white placeholder-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all duration-200"
                    />
                  ) : field.type === 'textarea' ? (
                    <textarea
                      id={field.id}
                      ...
                      rows={4}
                      required={field.required}
                      className="w-full px-4 py-3 bg-gray-800/70 border border-cyan-500/30 text-white placeholder-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 resize-none transition-all duration-200"
                    />
                  ) : field.type === 'select' ? (
                    <select
                      id={field.id}
                      name={field.id}
                      required={field.required}
                      className="w-full px-4 py-3 bg-gray-800/70 border border-cyan-500/30 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all duration-200"
                    >
              ...
              <div className="flex items-center justify-end space-x-4 pt-4 border-t border-gray-700">
                ...
                <button
                  type="submit"
                  className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-lg hover:from-cyan-600 hover:to-blue-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-gray-900 transition-all duration-200 shadow-lg shadow-cyan-500/25 flex items-center space-x-2"
                >
                  <Shield className="w-4 h-4" />
                  <span>{currentDialogConfig.submitText || 'Submit'}</span>
                </button>
              </div>
            </form>
          </>
